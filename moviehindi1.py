...
# st.columns
    ...
    #  st.image(img1,width=200)
      ...
      def play_songs(s):
             if (s):
              ...
              query = urllib.parse.quote(s + " official tariler")
  ...
  if 'songs_selectedh' not in st.session_state:
    st.session_state.songs_selectedh = random.sample(dmh.to_dict('records'), 9)  # Pick 2 random songs
        ...
        #  play_songs(s_l)
         def play(s_rhm):
            ...
            unsafe_allow_html=True,
                 ...
                 st.error("No video found for this smovieong 😢")
         play(s_rhm)
...
# 2 layout columns 
  ...
  def talk_ri(s_rihm):
        ...
        def play_songs(s_rihm):
            ...
            # talk_l(f"Playing the song {(song1[''])}")
      # url = f"https://www.google.com/search?q={podcast_name}"
            query = urllib.parse.quote(s_rihm + " official audio")
...
# Right Column - "Now Playing" Section
  ...
  def talk_mii(s_miihm):
        ...
        def play_songs(s_miihm):
            ...
            # talk_l(f"Playing the song {(song1[''])}")
      # url = f"https://www.google.com/search?q={podcast_name}"
            query = urllib.parse.quote(s_miihm + " official audio")
